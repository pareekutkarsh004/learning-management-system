...
function CDetails() {
  ...
  const fetchCourseData = async () => {
    try {
      ...
      // Try backendUrl first, fallback to hardcoded URL
      const apiUrl = backendUrl || "http://localhost:5000";
      console.log("Using API URL:", apiUrl);

      const { data } = await axios.get(`${apiUrl}/api/course/${id}`);
  ...
  const enrollCourse = async () => {
    ...
    try {
      ...
      console.log("🔍 Debug - isEnrolled:", isEnrolled);
      ...
      console.log("🔍 Debug - courseId:", course._id);

      const requestData = { courseId: course._id };
  ...
  useEffect(() => {
    if (userData && course) {
      // Fixed: Use 'enrolledCourses' instead of 'enrollCourse'
      ...
      console.log("🔍 Missing data for enrollment check:");
